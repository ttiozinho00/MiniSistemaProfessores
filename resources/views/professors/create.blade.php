...
<div class="container mx-auto mt-8">
    ...
    <form method="POST" action="{{ route('professors.store') }}" class="max-w-md">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-bold text-gray-700">Nome do Professor:</label>
        ...
        <!-- Add a text input for the 'matter' field -->
        <div class="mb-4">
            <label for="matter" class="block text-sm font-bold text-gray-700">Matéria:</label>
            <input type="text" name="matter" class="w-full px-4 py-2 border rounded-md @error('matter') border-red-500 @enderror" required>
            @error('matter')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
